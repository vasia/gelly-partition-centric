...
public class DBPediaLink {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Graph<Long, Long, NullValue> graph = Graph
                .fromCsvReader(
                        wrapper.getInputRoot() + "dbpedia-link/out.dbpedia-link.data",
        ...
        switch (args[0]) {
            case "pc":
                GraphCCRunner.detectComponentPC(
                        "DBPedia Partition Centric",
                        wrapper.getEnvironment(),
                        graph,
                        wrapper.getOutputRoot() + "pcdbpedia",
                        true,
                        1);
                break;
            case "vc":
                GraphCCRunner.detectComponentVC(
                        "DBPedia Vertex Centric",
                        wrapper.getEnvironment(),
                        graph,
                        wrapper.getOutputRoot() + "vcdbpedia",
                        true,
                        1);
                break;
            default:
    ...
    private static void printErr() {
        ...
        System.err.printf("Run \"java %s pc local\" for local partition-centric%n", DBPediaLink.class);
        System.err.printf("Run \"java %s vc local\" for local vertex-centric%n", DBPediaLink.class);
        System.err.printf("Run \"java %s pc remote\" for remote partition-centric%n", DBPediaLink.class);
        System.err.printf("Run \"java %s pc remote\" for remote partition-centric%n", DBPediaLink.class);
        System.exit(-1);
    }
}
